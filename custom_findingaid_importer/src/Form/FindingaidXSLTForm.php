...
/**                                                                                            
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    //handle file upload path configruation
    $org_fpath = $config->getOriginal('xslt_file_path');
    $overridden_fpath = $config->get('xslt_file_path');
    $form['Uploaded File Path'] = [
      '#type' => 'details',
      '#title' => $this->t('File System path to host XSLT Files uploaded'),
      '#open' => TRUE,
    ];
    $form['Uploaded File Path']['xslt_file_path'] = [
      '#type' => 'textfield',
      '#title' => $this->t('XSLT uploaded path'),
      '#default_value' => $config->get('xslt_file_path'),
      '#required' => TRUE,
    ];
        ...
        $form["xslt_aeon_files"][$i]['xslt_file'] =[
            '#type' => 'managed_file',
	          '#title' => $this->t('Upload a new XSLT to process Finding aids'),
		        ...
		        '#upload_location' => $config->get('xslt_file_path'),
                	'#required' => FALSE,
            ];
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $update_path = $form_state->getValue('xslt_file_path');
    $curr_path = $config->get('xslt_file_path');
    //new pair uploaded
    foreach ($form_state->getValue('xslt_aeon_files', []) as $key=>$file_data) {  
		  ...
		  if ($file) {
			...
			if ( $curr_template)  {
				...
				if ($prev_file) {
                 ...
                 //update uploaded file path if users change filepath configuration
                 if ( $update_path !== $curr_path) {
                    if( !file_exists($update_path) ) {
                       mkdir($update_path, 0775, TRUE);
                    }
                    $new_file_path = $update_path.'/'.$file->getFilename();
                    //move the uploaded file to the new file uploading location
                    $curr_file_path = $curr_path.'/'.$file->getFilename();
                    if( $curr_file_path && file_exists($curr_file_path) ) {
                        $file_to_move = \Drupal::entityTypeManager()->getStorage('file')->load($file->id());
                        $fs =\Drupal::Service('file_system');
                        $file_moved = $fs->move($file_to_move->getFileUri(),$new_file_path);
                        if ( $file_moved ) {
                           $file_to_move->setFileUri($new_file_path);
                           $file_to_move->setPermanent();
                           $file_to_move->save();
                        } else {
                          \Drupal::logger('custom_findingaid_importer')->error('Failed to move the file to the new configuration file path.');
                          return;
                        }
                    } else {  //it's a newfile, just set FileUri
                  	$file->setFileUri($new_file_path);
                    	$file->setPermanent();
                    	$file->save();
                    }
                  } else {
      ...
      $config->set('xslt_aeon_files', $updated_files)
              ->set('xslt_file_path', $update_path)
              ->save();
      ...
      $config_fid = str_replace('delete_', '', $config_del['#name']);	
	    ...
	    foreach($config_param as $key => $val) {
		    if ( isset($val['xslt_file']) && $val['xslt_file'] == $config_fid ) {
			      ...
			      $del_file = File::load($config_item['xslt_file']);                                               				    if ( $del_file ) {
			      		$del_file->delete();
                  		}
